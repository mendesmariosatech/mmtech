...
export function WalkTracker({
	...
	const startWalk = async () => {
		...
		const walkData = await onStartWalk(companyId, employeeId, selectedClient);
		if (walkData) {
	...
	const endWalk = async () => {
		...
		await onEndWalk(currentWalk.id, currentWalk.started_at, notes || undefined);

		setCurrentWalk(null);
		setNotes("");
		setElapsedTime(0);
		router.push("/dog-walking/employee");
	};
